...
function search(grepPattern) {
  var filesBlob = srcDir + '/*.md',
      grepStr = 'grep ' + grepPattern + " " + filesBlob,
      child = exec(grepStr, {silent:true}).output;

  if (!child.length) {
    return exit("No results found.");
  }
...
function trimmedEqually(str, matchedWord, maxLength) {
  var firstIdx = str.indexOf(matchedWord);
...
function highlightMatched(str, pattern) {
  var arr = str.split(" ");
  var idx = arr.indexOf(pattern);
  arr[idx] = arr[idx].underline.green;
  return arr.join(" ");
}
