...
v.meta = function(meta) {
  var ifc = { name: 'string', description: 'string', type: 'string', pattern: 'regexp', message: 'string', hidden: 'boolean',
   ...
   var allObjsValid = _.every(meta, function(val) {
      var keys = _.keys(val),
          allPropsValid = _.every(keys,function(key) {
...
I NEED TO REALLY WRAP MY HEAD AROUND WHAT I NEED HERE:


- Make a list of things that need validating -- literally one by one
- Then make a list of each step for each thing.
- Then generalize about what steps a validation module would need to take.
...
- it is valid JSON
- this screen is where it should be converted to JSON
- that meta has name, and that all other props are
